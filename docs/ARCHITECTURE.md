...
classDiagram
    ...
    note for Route "Default/Background/LongContext/Think drive\nrouting (internal/router.Select, wired in\nby cmd/ccr). LongContext fires on an\noversized prompt (>60000 est. tokens) and\napplies to BOTH inbound paths (v0.4.5): the\nOpenAI facade estimates its own body size.\nThink is live (v0.4.0): a request carrying a\nnon-null Anthropic thinking field routes to\nRouter.Think when configured — Anthropic-\ninbound (/v1/messages) only (no OpenAI\nequivalent field)."
...
| `Router.think` routing behaviour | Implemented and live (v0.4.0) — `translate.AnthropicRequest` models Anthropic's `thinking` field (`internal/translate/anthropic.go:51-60`), so a `POST /v1/messages` request carrying a non-null `thinking` block routes to `Router.Think` when configured (`requestWantsThinking`, `internal/router/selector.go:150-174`; `chooseRoute`, `router.go:164-175`). Anthropic-inbound only; the OpenAI facade has no `thinking` field to derive it from (long-context routing, however, applies to the facade as of v0.4.5) |
| Prometheus `/metrics` endpoint (self-contained, no `client_golang`) | Implemented and live (v0.4.0) — a shared `metrics.Recorder` (`internal/metrics`) the gateway records into, exposed as text-exposition `/metrics` on the loopback **management** server (`cmd/ccr/management.go:39-41`, `serve.go:45-62`, `internal/gateway/gateway.go:375-393`) |
| Semantic response-cache tier (`Cache.semantic`) | Implemented and live (v0.4.0), off by default — `gateway.BuildCache` wraps the exact tier in a `SemanticCache` + `LocalEmbedder` when `Cache.semantic` is set (`internal/gateway/gateway.go:194-235`); lexical near-duplicate, exact-first, scope-isolated, bounded |

...
- `cleancache` strips Anthropic-only `cache_control` blocks from the outgoing request. Upstreams that don't recognise the field tend to reject the **entire** request rather than ignore the unknown key, so every occurrence — at any nesting depth (system blocks, message content blocks, tool definitions) — has to go. `translate.StripCacheControl` walks the generic JSON tree recursively for exactly this reason (`internal/translate/anthropic.go:495-547`, tested at `internal/translate/anthropic_test.go:194-220`). It decodes with `json.Decoder.UseNumber()` rather than a plain `json.Unmarshal` into `any`, specifically so it never touches any number literal it passes through: a plain unmarshal converts every JSON number to `float64`, which both rejects any literal whose magnitude overflows `float64` (a fuzz-discovered case, `"1E700"`) and — worse — silently corrupts a large integer id like a 20-digit snowflake id into lossy scientific notation.
...
Both are opt-in per provider via `"transformer": {"use": [...]}`  (`internal/config/config.go:43-49`), and both are **fully wired end-to-end today**. The live `POST /v1/messages` handler reads `provider.Has("streamoptions")`/`provider.Has("cleancache")` directly into `Options.StreamOptions`/`Options.CleanCache` (`internal/gateway/messages.go:229-230`), and `AnthropicToOpenAI` acts on both.
...
3456 matches the Node implementation and every config already written by existing tooling — changing the *default* would break installed setups for no benefit (`internal/gateway/gateway.go:95-97`, comment at `internal/gateway/gateway_test.go:194-197`). It can, however, be overridden per-launch via `--gateway-port`/`CCR_GATEWAY_PORT` (`cmd/ccr/flags.go:29-37`) — useful when something else on the host already holds 3456 (commonly the Node `ccr` this project reimplements). See the next question.
...
- **Retry/fallback on a failed upstream call**: **implemented and live**. `internal/router/fallback.go`'s classification/backoff policy (`ClassifyStatus`, `ClassifyTransportError`, `FallbackRetryDelayAfterStatus`/`...AfterNetworkError`, honouring a `Retry-After` header) is now actually driven by a retry loop: `handleMessages` calls `doUpstreamWithRetry` (`internal/gateway/messages.go:294-383`) instead of making a single `s.Upstream.Do` call directly. It retries up to `Options.MaxAttempts` times total (default 3: one initial attempt plus two retries — `internal/gateway/gateway.go:47-53`, `65-68`), backing off between attempts, and **never** retries a `Terminal`-classified failure (e.g. a `401` — retrying it would only burn quota on a request that will fail identically every time). Once the budget is exhausted or a `Terminal` failure occurs, the response is forwarded exactly as before (see Q30). There is no CLI flag or `config.json` field yet to change `MaxAttempts` from its default.

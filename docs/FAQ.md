...
Not automatically, but the capability now exists as an **opt-in library function**: `internal/gateway.RequireAPIKey(keys []string)` returns a `gin.HandlerFunc` that accepts either `Authorization: Bearer <key>` or `x-api-key: <key>` (Anthropic's own SDKs send the latter), compared with `crypto/subtle.ConstantTimeCompare` so response timing can't leak how much of a guessed key was correct, and rejects everything else with a fixed `401` message that never echoes what the client sent (`internal/gateway/auth.go`). Its own doc comment is explicit that this is deliberately **not** installed automatically: passing an empty key list disables authentication entirely, "exactly as if this file did not exist" — required for backwards compatibility, since the toolkit driving this gateway today calls it with no client key configured on a loopback-only listener, and a default-deny would break every existing caller the moment authentication was wired in. Neither `gateway.go`'s route table nor `cmd/ccr` currently calls `RequireAPIKey` — so **the CLI-launched gateway is unauthenticated by default today**, same as before; a caller who wants it must explicitly install the middleware themselves. Anyone who can reach the gateway's port can send it requests billed to your configured provider keys until you do. See `docs/ADMIN_MANUAL.md` §5 for mitigations (bind to loopback, put an authenticating reverse proxy in front, or use `RequireAPIKey` yourself).

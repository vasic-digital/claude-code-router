...
- **`Router.think` and `Router.longContext` routing apply only to Anthropic-inbound requests.** Both are live as of v0.4.0 (`think`) and earlier (`longContext`): a `POST /v1/messages` request carrying a non-null `thinking` field routes to `Router.think`, and one whose estimated prompt exceeds `DefaultLongContextThreshold` (60000 tokens) routes to `Router.longContext`, when each is configured (`internal/router/selector.go:150-174`, `internal/router/router.go:130-175`). The OpenAI-inbound facade (`POST /v1/chat/completions`) builds an `AnthropicRequest` carrying only model+stream, so neither signal is derivable from that path and it routes on model alone — a documented future item (`internal/router/selector.go` scope caveat).
